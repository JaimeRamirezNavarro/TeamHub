...
<div class="max-w-5xl mx-auto">
    <div class="flex flex-col md:flex-row items-start md:items-center justify-between mb-10 border-b-4 border-black pb-6 gap-4">
        ...
        <a href="<?= BASE_PATH ?>/dashboard" class="border-2 border-black bg-white text-black font-bold uppercase py-2 px-4 shadow-brutal-sm hover:bg-neon hover:text-black active:translate-x-[2px] active:translate-y-[2px] active:shadow-none transition-none text-sm">
            <-- BACK_TO_ROOT
        </a>
    </div>
...
<script>
    ...
    function loadPresence() {
        fetch('<?= BASE_PATH ?>/api/online-users')
            .then(res => res.json())
            .then(data => {
