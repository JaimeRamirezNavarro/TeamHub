...
class TeamModel
{
    ...
    {
        // Admin: TODOS los equipos del sistema
        if ($role === 'admin') {
            return $this->db->query("SELECT * FROM teams")->fetchAll(PDO::FETCH_ASSOC);
        }
        
        // Manager: solo los equipos que ellos crearon
        if ($role === 'manager') {
            $stmt = $this->db->prepare("SELECT * FROM teams WHERE created_by = ?");
    ...
    public function obtenerMiembros($team_id)
    {
        $stmt = $this->db->prepare("
            ...
            WHERE tm.team_id = ? AND u.role != 'admin'
        ");
